...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('mesaieksilt')
    .setDescription('Bir kullanıcının mesai süresinden belirli bir süreyi eksilt')
    .addUserOption(option => 
      option.setName('kullanici')
        .setDescription('Mesai süresinden eksiltmek istediğiniz kullanıcı')
        .setRequired(true)
    )
    .addIntegerOption(option =>
      option.setName('saniye')
        .setDescription('Eksiltilecek süre (saniye)')
        .setRequired(true)
    ),
  async run({ interaction,client }) {
    try {
      ...
      mesaiData.startTime = new Date(currentTime - yeniTotalTime);

      // Veriyi dosyaya yaz
      saveMesaiData(targetUserId, mesaiData);

      await interaction.reply(`${targetUser.username}'ın mesai süresinden ${eksiltilecekSure} saniye eksiltildi. Yeni toplam mesai süresi: ${yeniTotalTime / 1000} saniye.`);
    } catch (error) {
      console.error('Hata oluştu:', error);
      interaction.reply('Kullanıcının mesai süresini eksiltirken bir hata oluştu.');
    }
  },
};
